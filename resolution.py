...
def resolution(file):
  (clauses, nbvars, nbclauses) = utils.parse_cnf(file)
  print('c Solving system from {0} with {1} variables and {2} clauses'.format(file, nbvars, nbclauses))

  start = time.perf_counter()
  result = solve(clauses)
  elapsed = time.perf_counter() - start
  print("s SATISFIABLE" if result else "s UNSATISFIABLE")
  print(f'Solved in: {elapsed:.6f} seconds with {len(clauses)} clauses')
  return (elapsed, len(clauses))
  ...
  if len(sys.argv) == 2:
    ...
    for dirname in dir_groups:
      ...
      for file in sorted(os.listdir(dir)):
        (elapsed, size) = resolution("{0}/{1}".format(dir, file))
        total_time = total_time + elapsed
        total_nbclauses = total_nbclauses + size
        count = count + 1
      results[dir] = (total_time / count, total_nbclauses / count)
    print("Results:")
    print(results)
